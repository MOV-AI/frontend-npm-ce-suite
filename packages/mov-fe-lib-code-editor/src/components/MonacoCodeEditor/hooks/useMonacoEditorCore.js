...
async function getBuiltins() {
  const defaultMonacoKind = languages.CompletionItemKind.Value;
...
function getSuggestions(model, position) {
  ...
  const builtinVars = Object.values(BUILTINS).filter(
    (builtin) => builtin.kind === languages.CompletionItemKind.Variable
  );
  ...
  for (let builtinVar of builtinVars) {
    if (lastWord.match(`${builtinVar.label}.`)) {
      suggestionsForObject = builtinVar.methods.map(
        ({ label, documentation }) => {
          // Put methods and attributes starting by "_" in the end of the suggestion queue
          const ordering = /^_{1,2}\w+_{1,2}$/.test(label)
            ? "z" + label
            : /^_\w+$/.test(label);
          const sortText = ordering ? "y" + label : label;
          // Suggestion for object
          return {
            label: label,
            insertText: label,
